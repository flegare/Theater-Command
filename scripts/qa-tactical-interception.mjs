...
async function run() {
  ...
  const battleBtn = page.locator(".custom-sector-popup button.sp-battle").first();
  if (await battleBtn.isVisible()) {
    ...
    // Expand preview
    const previewSummary = page.locator("summary:has-text('Preview Generated .ini')");
    ...
    const autoResolveTab = page.locator("button:has-text('Mode B: Instant Auto-Resolve')");
    if (await autoResolveTab.isVisible()) {
      await autoResolveTab.click();
      await page.waitForTimeout(600);
      console.log("Mode B active, clicking simulation button...");
      const simBtn = page.locator("button:has-text('Run Instant Auto-Resolve Simulation')");
